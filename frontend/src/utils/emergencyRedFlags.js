/**
 * Client-Side Emergency Red-Flag Engine & Audio Chime
 * Provides instantaneous, zero-latency detection of the 14 Emergency Categories
 * during both Text-to-Text and Voice-to-Voice patient check-up sessions.
 */

export const EMERGENCY_RED_FLAG_CATEGORIES = [
  {
    id: 'chest_heart',
    emoji: '❤️',
    ...
    alert: 'Possible acute coronary condition or myocardial distress. Requires immediate emergency cardiovascular triage.',
    ...
    regex: /(chest\s*pain|heart\s*pain|pressure\s*in\s*chest|tightness\s*in\s*chest|chest\s*pressure|crushing\s*chest|squeezing\s*chest|heaviness\s*in\s*chest|pain\s*(?:spreading|radiating)\s*to\s*(?:left\s*)?(?:arm|jaw|back|neck|shoulder)|left\s*arm\s*pain|chest\s*pain.*sweating|chest\s*pain.*nausea|chest\s*pain.*breathless|angina|heart\s*attack|छाती\s*में\s*(?:दर्द|दबाव)|सीने\s*में\s*(?:दर्द|जलन|दबाव)|दिल\s*का\s*दौरा|గుండె\s*నొప్పి|ఛాతీ\s*నొప్పి|மார்பు\s*வலி|छातीत\s*दुखणे)/i,
  },
  {
    id: 'breathing',
    emoji: '🫁',
    ...
    alert: 'Acute respiratory compromise or hypoxia. Requires immediate clinical airway assessment and supplemental oxygenation.',
  ...
  },
  {
    ...
    alert: 'Suspected acute ischemic or hemorrhagic stroke (FAST protocol). Time-critical emergency requiring rapid neuro-assessment.',
  ...
  },
  {
    ...
    alert: 'Possible subarachnoid hemorrhage or acute intracranial emergency requiring immediate neuroimaging.',
  ...
  },
  {
    ...
    alert: 'Acute hemorrhagic emergency or severe internal bleeding requiring rapid hemostasis and volume resuscitation.',
  ...
  },
  {
    ...
    alert: 'Syncope, acute coma, or hemodynamic collapse requiring rapid airway, breathing, circulation (ABC) evaluation.',
  ...
  },
  {
    ...
    alert: 'Status epilepticus or acute repetitive seizures. Carries risk of neurological injury. Immediate emergency care required.',
  ...
  },
  {
    ...
    alert: 'Major polytrauma or neurotrauma with risk of spinal damage, hemorrhage, or shock.',
  ...
  },
  {
    ...
    alert: 'Possible acute surgical abdomen (perforation, acute appendicitis, internal hemorrhage, or vascular compromise).',
  ...
  },
  {
    ...
    alert: 'Clinical suspicion of sepsis or acute systemic decompensation. Immediate intravenous therapy and monitoring required.',
  ...
  },
  {
    ...
    alert: 'Acute toxicological emergency requiring antidote evaluation and vital support.',
  ...
  },
  {
    ...
    alert: 'Critical obstetric emergency with maternal and fetal risk. Immediate emergency obstetric care required.',
  ...
  },
  {
    ...
    alert: 'Acute ophthalmic emergency with time-critical risk of permanent vision loss.',
  ...
  },
  {
    ...
    alert: 'Anaphylactic medical emergency with life-threatening airway obstruction risk. Epinephrine required.',
...
/**
 * Screen text in real-time on the client side
 * @param {string} text 
 * @returns {object}
 */
export const screenEmergencyRedFlags = (text = '') => {
  ...
  if (isRedFlag) {
    ...
    alertMessage = `🚨 RED FLAG EMERGENCY: [${names}] detected. Please seek immediate emergency medical care or call 112/108.`;
  }
...
/**
 * Synthesizes an audible hospital-grade emergency chime using Web Audio API
 */
export const playEmergencyAlertChime = () => {
  try {
    const AudioContext = window.AudioContext || window.webkitAudioContext;
    if (!AudioContext) return;
    const ctx = new AudioContext();

    // Two-tone urgent hospital attention chime (880 Hz High A -> 440 Hz A)
    ...
    osc2.type = 'triangle';
    osc2.frequency.setValueAtTime(587.33, now + 0.32); // D5
    osc2.frequency.exponentialRampToValueAtTime(440, now + 0.65); // A4
    ...
    console.warn('[Audio Alert] Chime playback notice:', err);
  }
};

/**
 * Returns localized emergency spoken warning text
 */
export const getSpokenEmergencyWarning = (language = 'en', primaryCategory) => {
  const catName = primaryCategory?.category || 'Medical Emergency';
  switch (language) {
    case 'hi':
      return `सावधान! ${catName} से संबंधित आपातकालीन लक्षण पाए गए हैं। कृपया तुरंत अस्पताल के आपातकालीन स्टाफ को बताएं या 112 पर कॉल करें।`;
    case 'te':
      return `హెచ్చరిక! ${catName} సంబంధిత అత్యవసర లక్షణాలు గుర్తించబడ్డాయి. దయచేసి వెంటనే అత్యవసర వైద్య సిబ్బందిని సంప్రదించండి లేదా 112 కు కాల్ చేయండి.`;
    case 'ta':
      return `எச்சரிக்கை! ${catName} தொடர்பான அவசர அறிகுறிகள் கண்டறியப்பட்டுள்ளன. தயவுசெய்து உடனடியாக மருத்துவமனை அவசர பிரிவை அணுகவும் அல்லது 112 ஐ அழைக்கவும்.`;
    case 'mr':
      return `सावधान! ${catName} संबंधी तातडीचे वैद्यकीय लक्षणे आढळली आहेत. कृपया त्वरित आपत्कालीन कक्षाशी संपर्क साधा किंवा 112 वर कॉल करा.`;
    case 'bn':
      return `সতর্কতা! ${catName} সম্পর্কিত জরুরি লক্ষণ ধরা পড়েছে। অনুগ্রহ করে অবিলম্বে জরুরি স্বাস্থ্য কর্মীদের জানান বা 112 নম্বরে কল করুন।`;
    default:
      return `Warning: High-priority emergency symptoms detected for ${catName}. Please alert hospital emergency triage staff immediately or call 112 or 108.`;
  }
};
